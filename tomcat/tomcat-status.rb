...
def usage
  puts __FILE__ + " <server> <port> <username> <password> <logdir>"
  exit 
end
...
protect do
  while true
    ...
    save_to_rolling_file "#{@tomcat_host}-tomcat-status", status_line    
    save_to_rolling_file "#{@tomcat_host}-tomcat-open-uris", add_time_stamp(working_url_list)
    sleep 10
  end
end
